...
export default function MapSearch({ defaultValue }: { defaultValue: string }) {
  ...
  return (
    <div className="relative mt-8 max-w-sm">
      <Search size={14} style={{ color: C.muted }} className="absolute start-3 top-1/2 -translate-y-1/2" />
      <input
        value={value}
        onChange={(e) => setValue(e.target.value)}
        placeholder={t("downloads.searchPlaceholder")}
        aria-label={t("downloads.searchPlaceholder")}
        className="w-full ps-9 pe-3 py-2.5 text-xs uppercase tracking-widest outline-none"
